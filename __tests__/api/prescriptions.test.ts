...
describe('POST /api/prescriptions (M8: psychiatrist-only)', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    resetRateLimiter();
  ...
  const validPayload = {
    fileUrl:
      'https://mockproject.supabase.co/storage/v1/object/prescriptions/user-1/rx_01.pdf',
    notes: 'Ajuste de dose a partir de segunda.',
  };
  ...
  const createRequest = (payload: unknown, token?: string, query = '') => {
    ...
    return new NextRequest(`http://localhost/api/prescriptions${query}`, {
      method: 'POST',
      headers,
  ...
  it('returns 403 for a clinician with the psychologist profile', async () => {
  ...
  it('returns 400 when fileUrl is on a foreign host (SSRF protection)', async () => {
    ...
    const res = await POST(
      createRequest(
        { ...validPayload, fileUrl: 'https://malicious.example/rx.pdf' },
        'token',
      ),
    );
    expect(res.status).toBe(400);
  ...
  it('inserts the prescription for the psychiatrist clinician', async () => {
    mockMember('clinician', 'psychiatrist', { userId: 'user-psiq-1' });
  ...
  it('accepts the admin preview via view_as=clinician&view_profile=psychiatrist', async () => {
    // The admin's real membership is owner; the substitution supplies both
    // the clinician role and the psychiatrist profile for this request.
  ...
  it('rejects the psychologist preview (403)', async () => {
    mockMember('owner', null, { admin: true });
    const res = await POST(
      createRequest(
        validPayload,
  ...
  it('rejects view_profile without view_as=clinician', async () => {
    mockMember('owner', null, { admin: true });
    const res = await POST(
      createRequest(validPayload, 'token', '?view_profile=psychiatrist'),
    );
    expect(res.status).toBe(403);
  });
});
